...
class SincronizadorAlegra:
    ...
    def main(self):
        ...
        self.logger.info("="*60)
        self.logger.info("INICIO DE SINCRONIZACIÓN ALEGRA-SHAREPOINT")
        self.logger.info("="*60)
        ...
        try:
            ...
            for pago_sp in pagos_sin_cliente:
                try:
                    self.procesar_pago_sin_cliente(pago_sp)
    ...
    def obtener_pagos_sin_cliente(self):
        ...
        try:
            ...
            # Obtener todos los pagos y filtrar en Python (más confiable)
            ...
            # Manejar paginación para obtener todos los registros
            while next_url:
                ...
                for item in all_pagos:
                    ...
                    if (not fields.get('ID_x0020_Cliente') or 
                        ...
                        pago_data = {
                            ...
                            'Pago_ID': fields.get('Title', ''),  # El Title contiene el Pago_ID de Alegra
                            ...
                            'fields': fields  # Guardar todos los campos para comparación
                        }
                        pagos.append(pago_data)
                
                self.logger.info(f"Encontrados {len(pagos)} pagos sin cliente en SharePoint")
                return pagos
            else:
                self.logger.error(f"Error obteniendo pagos: {response.status_code} - {response.text}")
                return []
    ...
    def procesar_pago_sin_cliente(self, pago_sp):
        """Procesar un pago específico sin cliente"""
        ...
        # Obtener datos actuales del pago desde Alegra
        ...
        # Verificar si el pago ahora tiene cliente
        cliente_alegra = self.safe_get_nested(pago_alegra, 'client', 'id', default='')
        
        if not cliente_alegra:
            self.logger.info(f"Pago {numero_pago} sigue sin cliente asignado en Alegra")
            self.stats['pagos_sin_cambios'] += 1
            return
        
        # El pago ahora tiene cliente, actualizar en SharePoint
        self.logger.info(f"Pago {numero_pago} ahora tiene cliente asignado, actualizando...")
        
        # Procesar datos del pago actualizado
        pagos_unificados = self.procesar_pago_alegra_unificado(pago_alegra)
        
        # Actualizar/reemplazar en SharePoint
        success = self.actualizar_pago_sharepoint(pago_sp, pagos_unificados)
        
        if success:
            self.stats['pagos_actualizados'] += 1
            
            # Si el pago tiene factura asociada, procesarla también
            id_factura = pago_sp.get('ID_Factura', '')
            if id_factura and id_factura.strip():
                self.logger.info(f"Pago tiene factura asociada {id_factura}, verificando actualización...")
                self.procesar_factura_asociada(id_factura)
        else:
            self.stats['pagos_error'] += 1
    ...
    def actualizar_pago_sharepoint(self, pago_sp, pagos_unificados):
        """Actualizar o reemplazar pago en SharePoint"""
        try:
            ...
            if self.eliminar_item_sharepoint(sharepoint_id, self.list_name_pagos):
                self.logger.info(f"Eliminado registro anterior del pago en SharePoint")
                ...
                if registros_creados > 0:
                    self.logger.info(f"Creados {registros_creados} nuevos registros para el pago")
                    return True
                else:
                    self.logger.error("No se pudieron crear los nuevos registros del pago")
                    return False
            else:
                self.logger.error("No se pudo eliminar el registro anterior")
                return False
                
        except Exception as e:
            self.logger.error(f"Error actualizando pago en SharePoint: {str(e)}")
            return False
    ...
    def eliminar_item_sharepoint(self, item_id, list_name):
        ...
        try:
            ...
            if response.status_code == 204:
                ...
                self.logger.error(f"Error eliminando item: {response.status_code} - {response.text}")
                return False
    ...
    def procesar_factura_asociada(self, factura_id):
        """Procesar factura asociada al pago"""
        try:
            self.logger.info(f"Verificando factura {factura_id} en Alegra...")
            
            # Obtener factura actual de Alegra
            ...
            # Obtener factura actual de SharePoint
            factura_sp = self.obtener_factura_sharepoint(factura_id)
            
            if not factura_sp:
                self.logger.warning(f"No se encontró factura {factura_id} en SharePoint")
                return
            
            # Comparar si hay cambios
            if self.factura_necesita_actualizacion(factura_sp, factura_alegra):
                self.logger.info(f"Factura {factura_id} necesita actualización")
                ...
                # Actualizar factura y sus items
                if self.actualizar_factura_sharepoint(factura_sp, factura_data, items_data):
                    self.stats['facturas_actualizadas'] += 1
                else:
                    self.stats['facturas_error'] += 1
            else:
                self.logger.info(f"Factura {factura_id} no necesita actualización")
                self.stats['facturas_sin_cambios'] += 1
                
        except Exception as e:
            self.logger.error(f"Error procesando factura asociada {factura_id}: {str(e)}")
            self.stats['facturas_error'] += 1
    ...
    def obtener_factura_sharepoint(self, factura_id):
        ...
        try:
            ...
            # Buscar por Title (que contiene el ID de Alegra)
            filter_query = f"fields/Title eq '{factura_id}'"
    ...
    def factura_necesita_actualizacion(self, factura_sp, factura_alegra):
        """Comparar factura de SharePoint con la de Alegra para ver si necesita actualización"""
        try:
            fields_sp = factura_sp['fields']
            ...
            comparaciones = [
                ...
                ('Total_x0020_Pagado', factura_alegra.get('totalPaid', 0)),
                ('Cliente_x0020_Nombre', self.safe_get_nested(factura_alegra, 'client', 'name', default='')),
            ]
            
            for campo_sp, valor_alegra in comparaciones:
                valor_sp = fields_sp.get(campo_sp, '')
                
                # Normalizar valores para comparación
                if isinstance(valor_sp, (int, float)) and isinstance(valor_alegra, (int, float)):
                    if abs(valor_sp - valor_alegra) > 0.01:  # Tolerancia para decimales
                        self.logger.info(f"Cambio detectado en {campo_sp}: SP={valor_sp} vs Alegra={valor_alegra}")
                        return True
                else:
                    if str(valor_sp).strip() != str(valor_alegra).strip():
                        self.logger.info(f"Cambio detectado en {campo_sp}: SP='{valor_sp}' vs Alegra='{valor_alegra}'")
                        return True
            ...
            self.logger.error(f"Error comparando factura: {str(e)}")
            return True
    ...
    def actualizar_factura_sharepoint(self, factura_sp, factura_data, items_data):
        """Actualizar factura completa en SharePoint (eliminar y recrear)"""
        try:
            factura_id = factura_data['ID']
            sharepoint_id = factura_sp['SharePoint_ID']
            
            self.logger.info(f"Actualizando factura {factura_id} en SharePoint...")
            
            # Paso 1: Eliminar items actuales de la factura
            items_eliminados = self.eliminar_items_factura(factura_id)
            self.stats['items_eliminados'] += items_eliminados
            
            # Paso 2: Eliminar factura actual
            if self.eliminar_item_sharepoint(sharepoint_id, self.list_name_facturas):
                self.logger.info(f"Eliminada factura anterior de SharePoint")
                
                # Paso 3: Crear factura nueva
                nuevo_id_factura = self.crear_factura_sharepoint(factura_data)
                
                if nuevo_id_factura:
                    self.logger.info(f"Creada nueva factura con ID: {nuevo_id_factura}")
                    ...
                    self.stats['items_actualizados'] += items_creados
                    self.logger.info(f"Creados {items_creados} items para la factura")
                    
                    return True
                else:
                    self.logger.error("No se pudo crear la nueva factura")
                    return False
            else:
                self.logger.error("No se pudo eliminar la factura anterior")
                return False
                
        except Exception as e:
            self.logger.error(f"Error actualizando factura en SharePoint: {str(e)}")
            return False
    ...
    def eliminar_items_factura(self, factura_id):
        ...
        try:
            ...
            # Buscar items por Factura_ID (que está en el Title de la factura)
            filter_query = f"fields/Title eq '{factura_id}'"
            url = f"https://graph.microsoft.com/v1.0/sites/{site_id}/lists/{list_id}/items?$expand=fields&$filter={filter_query}"
            ...
            self.logger.info(f"Eliminados {items_eliminados} items de la factura")
    ...
    def mostrar_resumen_final(self):
        ...
        self.logger.info("="*60)
        self.logger.info("RESUMEN FINAL DE SINCRONIZACIÓN")
        self.logger.info("="*60)
        self.logger.info(f"PAGOS:")
        self.logger.info(f"  - Revisados: {self.stats['pagos_revisados']}")
        self.logger.info(f"  - Actualizados: {self.stats['pagos_actualizados']}")
        ...
        self.logger.info(f"  - Actualizadas: {self.stats['facturas_actualizadas']}")
        self.logger.info(f"  - Sin cambios: {self.stats['facturas_sin_cambios']}")
        self.logger.info(f"  - Con errores: {self.stats['facturas_error']}")
        
        self.logger.info(f"ITEMS:")
        self.logger.info(f"  - Actualizados: {self.stats['items_actualizados']}")
        self.logger.info(f"  - Eliminados: {self.stats['items_eliminados']}")
        
        # También mostrar en consola
        print(f"Sincronización completada:")
        print(f"  Pagos actualizados: {self.stats['pagos_actualizados']}")
        print(f"  Facturas actualizadas: {self.stats['facturas_actualizadas']}")
        print(f"  Items procesados: {self.stats['items_actualizados']}")
...
def main():
    ...
    try:
        print("Iniciando sincronización Alegra-SharePoint...")
